...
(function () {
  ...
  /* ══ 11. MOBILE OPTIMIZATIONS ════════════════════════════
     Enhance touch interactions, prevent zoom, optimize video playback
  ══════════════════════════════════════════════════════════ */
  ...
  if (isMobile) {
    ...
    $$('button, a, .proj-card, .filter-btn').forEach(el => {
      el.addEventListener('touchend', e => {
        if (e.timeStamp - lastTouchTime < 500) e.preventDefault();
        lastTouchTime = e.timeStamp;
      }, false);
    });

    // Improve video loading on mobile with loading indicator
    if (modalVideo) {
      ...
      modalVideo.addEventListener('error', () => {
        showToast('Video failed to load. Check your connection.', 'error');
        modal.classList.remove('video-loading');
      });
    }

    // Adjust modal for keyboard on mobile (iOS)
    ...
    // Fallback: Add loading state to video modal
  ...
  // Improve form input on mobile (prevent zoom on input focus)
  if (isSmallScreen) {
    $$('input, textarea, select').forEach(input => {
      input.addEventListener('focus', () => {
        // Disable zoom on input focus by setting font-size to 16px temporarily
